...
-EMPLOYEES, DEPARTMENTS 테이블을 엘리어스(별칭)를 이용해서
...
--내가 작성
SELECT *
FROM employees e JOIN departments d --INNER 적음
ON e.department_id = d.department_id;

SELECT *
FROM employees e
LEFT JOIN departments d --INNER 적음
ON e.department_id = d.department_id;
...
SELECT *
FROM employees e
JOIN departments d --(INNER) 생략 가능
ON e.department_id = d.department_id
...
ON e.department_id = d.department_id
JOIN locations loc
ON d.location_id = loc.location_id --요거 빼먹음
WHERE e.job_id = 'SA_MAN'
ORDER BY 사원번호;
...
AND j.job_title = 'Stock Clerk'; -- OR 잘못적음
...
FROM employees e1
LEFT JOIN employees e2 --결국 매니져도 사원임
...
--내가 작성 : 아이디 없는 사람 쓰려면 left join 쓰면 됨
SELECT
    e1.manager_id, e1.first_name, e1.salary
FROM employees e1
