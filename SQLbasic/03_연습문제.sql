...
SELECT * 
FROM employees
WHERE department_id = 50; -- '50' 으로 적음
...
SELECT  first_name || ' ' || last_name AS NAME, salary+300 AS SALARY
FROM employees;
...
SELECT first_name || ' ' || last_name AS NAME, job_id
FROM employees
WHERE job_id IN ('IT_PROG', 'SA_MAN'); --동일
--12번까지 풀음ㅠ

--13. Steven King 사원의 정보를 “Steven King 사원의 급여는 24000달러 입니다” 형식으로 출력하세요.
SELECT first_name || ' ' || last_name || ' 사원의 급여는 ' || salary || ' 달러 입니다.' AS test
FROM employees
